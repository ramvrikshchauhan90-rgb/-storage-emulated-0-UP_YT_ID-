...
def newinfo(uid):
    ...
    params = {
        'uid': uid,
        'server': 39699,  # Hardcoded to bd as requested
        'key': key2
    }
    try:
...
async def EncRypTMajoRLoGin(open_id, access_token):
    ...
    major_login.client_ip = "202.81.109.200"
...
async def TcPOnLine(ip, port, key, iv, AutHToKen, reconnect_delay=0.5):
    global online_writer , spam_room , whisper_writer , spammer_uid , spam_chat_id , spam_uid , XX , uid , Spy,data2, Chat_Leave, fast_spam_running, fast_spam_task, custom_spam_running, custom_spam_task, spam_request_running, spam_request_task
    while True:
        try:
...
async def TcPChaT(ip, port, AutHToKen, key, iv, LoGinDaTaUncRypTinG, ready_event, region , reconnect_delay=0.5):
    ...
    global spam_room , whisper_writer , spammer_uid , spam_chat_id , spam_uid , online_writer , chat_id , XX , uid , Spy,data2, Chat_Leave, fast_spam_running, fast_spam_task, custom_spam_running, custom_spam_task, spam_request_running, spam_request_task
    while True:
        try:
            reader , writer = await asyncio.open_connection(ip, int(port))
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    try:
                        ...
                        print(f"Received message: {inPuTMsG} from UID: 12270613470 in chat type: 1")
...
[b][i][FFC0CB]youtube: UP KiNG BOSS[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @upkingboss[/b]

[b][c][FFA500]telegram contact: @upkingboss[A52A2A]
 
Enjoy the bot my friend.......

[C][B][0000FF] Created by UP KiNG BOSS
Modified by - UP
"""
                        ...
                        if inPuTMsG.startswith('/solo'):
                            # Process /solo command in any chat type
                        ...
                        if inPuTMsG.strip().startswith('/spm'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /spm (uid)\nExample: /spm 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                try:
                                    target_uid = parts[1]
                        ...
                        # NEW 4-MENU HELP SYSTEM
                        if inPuTMsG in ("op" , "/Raihan" , "hi" , "/help"):
                            # Menu 1
                            menu1 = '''[B][C][FFFFFF]FREE F[C][B][FFD700]I[B][C][FFFFFF]RE

[FFFFFF]Hey [FFFF00] Users ❤️
[FFFFFF]Welcome to Raihan Bot
[C][B][FF0000]━━━━?MENU?━━━━

[C][B][FFFF00]Available Commands:
[B][C][FFFFFF]/🗿3 [00FF00]- 3 Player Group
[B][C][FFFFFF]/🗿5 [00FF00]- 5 Player Group
[B][C][FFFFFF]/🗿6 [00FF00]- 6 Player Group
[B][C][FFFFFF]/🗿inv [uid]  [00FF00]- Invite Player'''
                            await safe_send_message(response.Data.chat_type, menu1, uid, chat_id, key, iv)
                            
                            await asyncio.sleep(1)  # Small delay between menus
                            
                            # Menu 2
                            menu2 = '''[C][B][FF0000]━━━━MENU━━━━

[C][B][FFFF00]Available Commands:
[B][C][FFFFFF]/🗿x/ [teamcode] [00FF00]- Join Teamcode
[B][C][FFFFFF]/🗿spm/ [uid] [00FF00]- spam invite
[B][C][FFFFFF]/🗿solo [00FF00]- leave Group
[B][C][FFFFFF]/🗿 admin [00FF00]- Admin Details'''
                            await safe_send_message(response.Data.chat_type, menu2, uid, chat_id, key, iv)
                            
                            await asyncio.sleep(1)  # Small delay between menus
                            
                            # Menu 3
                            menu3 = '''[C][B][FF0000]━━━ ROOM MENU ━━━

UPCOMING 

[C][B][FFFF00]Room Commands:
[B][C][FFFFFF]/🗿room_join [room_id] [pass] [00FF00]- Bot join coustom
[B][C][FFFFFF]/🗿leave_room [room_id] [00FF00]- Bot leave coustom'''
                            await safe_send_message(response.Data.chat_type, menu3, uid, chat_id, key, iv)
                            
                            await asyncio.sleep(1)  # Small delay between menus
                            
                            # Menu 4
                            menu4 = '''[C][B][FFFF00]Goup Emote Commands:
[B][C][FFFFFF]/🗿e [uid] [emote_id] [00FF00]- Group Emote
[B][C][FFFFFF]/🗿 fast [uid] [emote_id] [00FF00]- Fast emote
[B][C][FFFFFF]/🗿p [uid] [emote_id] [time] [00FF00]- Fast emote 1-100

[C][B][FFB300]OWNER: UP KiNG BOSS 
[00FFFF]━━━━━━━━━━━━
...
async def MaiiiinE():
    Uid , Pw = '4242524222','65FB035D7FB9ACB5F2236876A7E3D2DC250AF6F1B723A1BA0FB076ACCE3A0DF6'
    ...
    print(f" - BoT sTaTus > GooD | UP KiNG BOSS  ! (:")    
    print(f" - Subscribe > UP KiNG BOSS  | Gaming ! (:")    
